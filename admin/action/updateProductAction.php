...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $bouquet_stock = $_POST['bouquet_stock'];
    ...
    if ($bouquet_image != "") {
        ...
        if ($error === 0) {
            ...
            if (in_array($image_ex, $allowed_exs)) {
                ...
                $query = "UPDATE tb_produk SET bouquet_code = ?, bouquet_name = ?, bouquet_image = ?, bouquet_description = ?, bouquet_type = ?, bouquet_price = ?, bouquet_stock = ?, bouquet_ratings = ?, bouquet_category = ? WHERE bouquet_id = ?";
                $stmt = $conn->prepare($query);
                $stmt->bind_param("sssssiidsi", $bouquet_code, $bouquet_name, $bouquet_image, $bouquet_description, $bouquet_type, $bouquet_price, $bouquet_stock, $bouquet_ratings, $bouquet_category, $bouquet_id);
                $stmt->execute();
                $stmt->close();
        ...
        $query = "UPDATE tb_produk SET bouquet_code = ?, bouquet_name = ?, bouquet_description = ?, bouquet_type = ?, bouquet_price = ?, bouquet_stock = ?, bouquet_ratings = ?, bouquet_category = ? WHERE bouquet_id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssiidsi", $bouquet_code, $bouquet_name, $bouquet_description, $bouquet_type, $bouquet_price, $bouquet_stock, $bouquet_ratings, $bouquet_category, $bouquet_id);
        $stmt->execute();
        $stmt->close();
